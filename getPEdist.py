...
res0Id = 0
#note if reading CG traj, each chain is a residue thus don't need to count residues with resCount and dont need -dop inputs

def GetPEDist(traj, top, stride, MolNames, NPs, nbins, Ls_in):
...
#                if resCount%DOP == 0.:
    ...
    for MolName,AtomsInMol in AtomsInMolDict.items():
        ...
        for atomId in AtomsInMol: #loop through molecules in with this MolName
            xyz = traj.atom_slice(atomId).xyz
            COM =  np.sum(xyz, axis = 1)
            COM /= xyz.shape[1] #divide by number of beads in each molecule
            COMs.append(COM)
...
#    DOPs = args.dop
    ...
    nbins = args.nbins
    Ls_in = args.L
    GetPEDist(traj, top, stride, MolNames, NPs, nbins, Ls_in)
