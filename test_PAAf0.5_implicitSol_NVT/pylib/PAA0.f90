...
module m
    ...
    integer, parameter :: NDParam = 49
    integer, parameter :: NDDParam = 281
    ...
    integer, parameter :: P14_ArgHistNBin = 10000
    ...
    integer, parameter :: M10_NVal = 49
    integer, parameter :: M11_NVal = 281
    integer, parameter :: M12_NVal = 49
    integer, parameter :: M13_NVal = 281
    integer, parameter :: M14_NVal = 2401
    ...
    integer :: TargetAtom
    integer :: TargetMol
    real(8), dimension(0:15-1) :: Terms
    real(8), dimension(0:15-1) :: Cut
    real(8), dimension(0:15-1) :: CutSq
    real(8) :: OldPEnergy
    real(8), dimension(0:15-1) :: OldTerms
    real(8) :: FluctE
    real(8) :: FluctE0
    real(8) :: FluctA0
    real(8) :: FluctBeta
    real(8) :: FluctTerm
    real(8), dimension(0:49-1) :: Param
    real(8), dimension(0:49-1) :: DUParam
    real(8), dimension(0:49-1) :: DWParam
    real(8), dimension(0:281-1) :: DDUParam
    real(8), dimension(0:281-1) :: DDWParam
    ...
    real(8) :: P14_PlaneLoc
    integer :: P14_PlaneAxis
    real(8) :: P14_ArgWeightSumHist
    real(8) :: P14_ArgWeightSumStats
    real(8), dimension(0:1-1) :: P14_ArgMin
    real(8), dimension(0:1-1) :: P14_ArgMax
    real(8), dimension(0:1-1) :: P14_ArgCount
    real(8), dimension(0:1-1) :: P14_ArgSum
    real(8), dimension(0:1-1) :: P14_ArgSumSq
    real(8), dimension(0:1-1) :: P14_ArgReportMin
    real(8), dimension(0:1-1) :: P14_ArgReportMax
    real(8), dimension(0:1-1) :: P14_ArgReportDelta
    real(8), dimension(0:1-1) :: P14_ArgHistMin
    real(8), dimension(0:1-1) :: P14_ArgHistMax
    real(8), dimension(0:1-1) :: P14_ArgHistBinw
    real(8), dimension(0:1-1) :: P14_ArgHistiBinw
    real(8), dimension(0:1-1, 0:10000-1) :: P14_ArgHist
    integer :: M0_StepFreq
    ...
    logical :: M10_Active
    real(8), dimension(0:49-1) :: M10_Val
    real(8), dimension(0:49-1) :: M10_ValSum
    real(8), dimension(0:49-1, 0:49-1) :: M10_ValSumSq
    real(8) :: M10_Count
    integer :: M11_StepFreq
    integer :: M11_CycleFreq
    logical :: M11_Active
    real(8), dimension(0:281-1) :: M11_Val
    real(8), dimension(0:281-1) :: M11_ValSum
    real(8), dimension(0:281-1) :: M11_ValSumSq
    real(8) :: M11_Count
    integer :: M12_StepFreq
    integer :: M12_CycleFreq
    logical :: M12_Active
    real(8), dimension(0:49-1) :: M12_Val
    real(8), dimension(0:49-1) :: M12_ValSum
    real(8), dimension(0:49-1) :: M12_ValSumSq
    real(8) :: M12_Count
    integer :: M13_StepFreq
    integer :: M13_CycleFreq
    logical :: M13_Active
    real(8), dimension(0:281-1) :: M13_Val
    real(8), dimension(0:281-1) :: M13_ValSum
    real(8), dimension(0:281-1) :: M13_ValSumSq
    real(8) :: M13_Count
    integer :: M14_StepFreq
    integer :: M14_CycleFreq
    logical :: M14_Active
    real(8), dimension(0:2401-1) :: M14_Val
    real(8), dimension(0:2401-1) :: M14_ValSum
    real(8), dimension(0:2401-1) :: M14_ValSumSq
    real(8) :: M14_Count
...
subroutine calcenergyforces(Mode, CalcForce, CalcVirial, CalcDUParam, CalcDWParam, CalcFluct)
    ...
    if (CALCDUPARAM .or. CALCDWPARAM) then
        ...
        do i = iStart, iStop
            ...
            do k = 0, P7_NKVec-1
                ...
                DDUParam(251 + AIDI + 0) = DDUParam(251 + AIDI + 0) + val1 * val4
                if (CALCDWPARAM) then
                    DWParam(AIDI) = DWParam(AIDI) + val2 * val4 * val5
                    DDWParam(251 + AIDI + 0) = DDWParam(251 + AIDI + 0) + val1 * val4 * val5
                endif
                do j = i, iStop
                    AIDJ = AID(j)
                    if (i == j) then
            ...
            DDUParam(251 + AIDI + 0) = DDUParam(251 + AIDI + 0) + val3 * Param(AIDI)
        enddo
    endif
    ...
    do i = istart, istop
        ...
        if (AIDi==1) then
            ...
            PlaneDist = (POSIMINIMAGE(2) - P14_PlaneLoc)
            if (BOXL(2)> 0.d0) PlaneDist = PlaneDist - BOXL(2) * dnint(PlaneDist * IBOXL(2))
            factor = 2*PI*Param(48)/BOXL(2)
            !THISU = Param(47) * SIN(2*PI*(PlaneDist-P14_PlaneLoc)*Param(48)/BOXL(2))
            THISU = Param(47) * SIN(factor*(PlaneDist-P14_PlaneLoc))
            !THISU = Param(47) * PlaneDist
            PEnergy = PEnergy + ThisU * Scale
            Terms(14) = Terms(14) + ThisU * Scale
            if (CalcDUParam) then
            !    DUParam(47) = DUParam(47) + SIN(2*PI*(PlaneDist-P14_PlaneLoc)*Param(48)/BOXL(2))
                DUParam(47) = DUParam(47) + SIN(factor*(PlaneDist-P14_PlaneLoc))
            endif
            if (CalcForce) then
                FORCEI = 0.d0
            !    FORCEI(2) = -Param(47) * 2*PI*Param(48)/BoxL(2) * COS(2*PI*(PlaneDist-P14_PlaneLoc)*Param(48)/BOXL(2))
                FORCEI(2) = -Param(47) * factor * COS(factor*(PlaneDist-P14_PlaneLoc))
            !    ForceI(2) = - Param(47)
            ...
            if (dijsq < CutSq(7)) then
                ...
                if (CALCDUPARAM) then
                    ...
                    if (AIDI==AIDJ) then
                        ...
                        DDUParam(251 + AIDI + 0) = DDUParam(251 + AIDI + 0) + 2.d0 * Chargej * val2
                        ...
                        DDUParam(251 + AIDI + 0) = DDUParam(251 + AIDI + 0) + Chargej * val2
                        DDUParam(251 + AIDJ + 0) = DDUParam(251 + AIDJ + 0) + Chargei * val2
                ...
                if (CALCDWPARAM) then
                    ...
                    if (AIDI==AIDJ) then
                        ...
                        DDWParam(251 + AIDI + 0) = DDWParam(251 + AIDI + 0) + 2.d0 * Chargej * val3
                        ...
                        DDWParam(251 + AIDI + 0) = DDWParam(251 + AIDI + 0) + Chargej * val3
                        DDWParam(251 + AIDJ + 0) = DDWParam(251 + AIDJ + 0) + Chargei * val3
                ...
                if (((AIDi==1) .and. (AIDj==1)) .or. ((AIDj==1) .and. (AIDi==1))) then
                    ...
                    if (CALCDUPARAM) then
                        ...
                        if (AIDI==AIDJ) then
                            DUParam(AIDI) = DUParam(AIDI) + 2.d0 * Param(35) * Chargej * (val2 + P8_UShift(0) - idist + &
                              & P8_CoulShift(0))
                            DDUParam(255 + AIDI + 0) = DDUParam(255 + AIDI + 0) + 2.d0 * Param(35) * Chargej * (-val5 + &
                              & P8_UShift(1))
                            DDUParam(253 + AIDI + 0) = DDUParam(253 + AIDI + 0) + 2.d0 * Chargej * (val2 + P8_UShift(0) - idist &
                              & + P8_CoulShift(0))
                            ...
                            DDUParam(255 + AIDI + 0) = DDUParam(255 + AIDI + 0) + Param(35) * Chargej * (-val5 + P8_UShift(1))
                            DDUParam(255 + AIDJ + 0) = DDUParam(255 + AIDJ + 0) + Param(35) * Chargei * (-val5 + P8_UShift(1))
                            DDUParam(253 + AIDI + 0) = DDUParam(253 + AIDI + 0) + Chargej * (val2 + P8_UShift(0) - idist + &
                              & P8_CoulShift(0))
                            DDUParam(253 + AIDJ + 0) = DDUParam(253 + AIDJ + 0) + Chargei * (val2 + P8_UShift(0) - idist + &
                              & P8_CoulShift(0))
                    ...
                    if (CALCDWPARAM) then
                        ...
                        if (AIDI==AIDJ) then
                            ...
                            DDWParam(255 + AIDI + 0) = DDWParam(255 + AIDI + 0) + 2.d0 * Chargej * Param(35) * val4
                            ...
                            DDWParam(255 + AIDI + 0) = DDWParam(255 + AIDI + 0) + Param(35) * val4 * Chargej
                            DDWParam(255 + AIDJ + 0) = DDWParam(255 + AIDJ + 0) + Param(35) * val4 * Chargei
                ...
                if (((AIDi==1) .and. (AIDj==0)) .or. ((AIDj==1) .and. (AIDi==0))) then
                    ...
                    if (CALCDUPARAM) then
                        ...
                        if (AIDI==AIDJ) then
                            DUParam(AIDI) = DUParam(AIDI) + 2.d0 * Param(37) * Chargej * (val2 + P9_UShift(0) - idist + &
                              & P9_CoulShift(0))
                            DDUParam(259 + AIDI + 0) = DDUParam(259 + AIDI + 0) + 2.d0 * Param(37) * Chargej * (-val5 + &
                              & P9_UShift(1))
                            DDUParam(257 + AIDI + 0) = DDUParam(257 + AIDI + 0) + 2.d0 * Chargej * (val2 + P9_UShift(0) - idist &
                              & + P9_CoulShift(0))
                            ...
                            DDUParam(259 + AIDI + 0) = DDUParam(259 + AIDI + 0) + Param(37) * Chargej * (-val5 + P9_UShift(1))
                            DDUParam(259 + AIDJ + 0) = DDUParam(259 + AIDJ + 0) + Param(37) * Chargei * (-val5 + P9_UShift(1))
                            DDUParam(257 + AIDI + 0) = DDUParam(257 + AIDI + 0) + Chargej * (val2 + P9_UShift(0) - idist + &
                              & P9_CoulShift(0))
                            DDUParam(257 + AIDJ + 0) = DDUParam(257 + AIDJ + 0) + Chargei * (val2 + P9_UShift(0) - idist + &
                              & P9_CoulShift(0))
                    ...
                    if (CALCDWPARAM) then
                        ...
                        if (AIDI==AIDJ) then
                            ...
                            DDWParam(259 + AIDI + 0) = DDWParam(259 + AIDI + 0) + 2.d0 * Chargej * Param(37) * val4
                            ...
                            DDWParam(259 + AIDI + 0) = DDWParam(259 + AIDI + 0) + Param(37) * val4 * Chargej
                            DDWParam(259 + AIDJ + 0) = DDWParam(259 + AIDJ + 0) + Param(37) * val4 * Chargei
                ...
                if (((AIDi==1)) .or. ((AIDj==1))) then
                    ...
                    if (CALCDUPARAM) then
                        ...
                        if (AIDI==AIDJ) then
                            DUParam(AIDI) = DUParam(AIDI) + 2.d0 * Param(39) * Chargej * (val2 + P10_UShift(0) - idist + &
                              & P10_CoulShift(0))
                            DDUParam(263 + AIDI + 0) = DDUParam(263 + AIDI + 0) + 2.d0 * Param(39) * Chargej * (-val5 + &
                              & P10_UShift(1))
                            DDUParam(261 + AIDI + 0) = DDUParam(261 + AIDI + 0) + 2.d0 * Chargej * (val2 + P10_UShift(0) - &
                            ...
                            DUParam(AIDJ) = DUParam(AIDJ) + Param(39) * Chargei * (val2 + P10_UShift(0) -idist + &
                              & P10_CoulShift(0))
                            DDUParam(263 + AIDI + 0) = DDUParam(263 + AIDI + 0) + Param(39) * Chargej * (-val5 + P10_UShift(1))
                            DDUParam(263 + AIDJ + 0) = DDUParam(263 + AIDJ + 0) + Param(39) * Chargei * (-val5 + P10_UShift(1))
                            DDUParam(261 + AIDI + 0) = DDUParam(261 + AIDI + 0) + Chargej * (val2 + P10_UShift(0) - idist + &
                              & P10_CoulShift(0))
                            DDUParam(261 + AIDJ + 0) = DDUParam(261 + AIDJ + 0) + Chargei * (val2 + P10_UShift(0) - idist + &
                              & P10_CoulShift(0))
                    ...
                    if (CALCDWPARAM) then
                        ...
                        if (AIDI==AIDJ) then
                            ...
                            DDWParam(263 + AIDI + 0) = DDWParam(263 + AIDI + 0) + 2.d0 * Chargej * Param(39) * val4
                            ...
                            DDWParam(263 + AIDI + 0) = DDWParam(263 + AIDI + 0) + Param(39) * val4 * Chargej
                            DDWParam(263 + AIDJ + 0) = DDWParam(263 + AIDJ + 0) + Param(39) * val4 * Chargei
                ...
                if (((AIDi==0) .and. (AIDj==0)) .or. ((AIDj==0) .and. (AIDi==0))) then
                    ...
                    if (CALCDUPARAM) then
                        ...
                        if (AIDI==AIDJ) then
                            DUParam(AIDI) = DUParam(AIDI) + 2.d0 * Param(41) * Chargej * (val2 + P11_UShift(0) - idist + &
                              & P11_CoulShift(0))
                            DDUParam(267 + AIDI + 0) = DDUParam(267 + AIDI + 0) + 2.d0 * Param(41) * Chargej * (-val5 + &
                              & P11_UShift(1))
                            DDUParam(265 + AIDI + 0) = DDUParam(265 + AIDI + 0) + 2.d0 * Chargej * (val2 + P11_UShift(0) - &
                            ...
                            DUParam(AIDJ) = DUParam(AIDJ) + Param(41) * Chargei * (val2 + P11_UShift(0) -idist + &
                              & P11_CoulShift(0))
                            DDUParam(267 + AIDI + 0) = DDUParam(267 + AIDI + 0) + Param(41) * Chargej * (-val5 + P11_UShift(1))
                            DDUParam(267 + AIDJ + 0) = DDUParam(267 + AIDJ + 0) + Param(41) * Chargei * (-val5 + P11_UShift(1))
                            DDUParam(265 + AIDI + 0) = DDUParam(265 + AIDI + 0) + Chargej * (val2 + P11_UShift(0) - idist + &
                              & P11_CoulShift(0))
                            DDUParam(265 + AIDJ + 0) = DDUParam(265 + AIDJ + 0) + Chargei * (val2 + P11_UShift(0) - idist + &
                              & P11_CoulShift(0))
                    ...
                    if (CALCDWPARAM) then
                        ...
                        if (AIDI==AIDJ) then
                            ...
                            DDWParam(267 + AIDI + 0) = DDWParam(267 + AIDI + 0) + 2.d0 * Chargej * Param(41) * val4
                            ...
                            DDWParam(267 + AIDI + 0) = DDWParam(267 + AIDI + 0) + Param(41) * val4 * Chargej
                            DDWParam(267 + AIDJ + 0) = DDWParam(267 + AIDJ + 0) + Param(41) * val4 * Chargei
                ...
                if (((AIDi==0)) .or. ((AIDj==0))) then
                    ...
                    if (CALCDUPARAM) then
                        ...
                        if (AIDI==AIDJ) then
                            DUParam(AIDI) = DUParam(AIDI) + 2.d0 * Param(43) * Chargej * (val2 + P12_UShift(0) - idist + &
                              & P12_CoulShift(0))
                            DDUParam(271 + AIDI + 0) = DDUParam(271 + AIDI + 0) + 2.d0 * Param(43) * Chargej * (-val5 + &
                              & P12_UShift(1))
                            DDUParam(269 + AIDI + 0) = DDUParam(269 + AIDI + 0) + 2.d0 * Chargej * (val2 + P12_UShift(0) - &
                            ...
                            DUParam(AIDJ) = DUParam(AIDJ) + Param(43) * Chargei * (val2 + P12_UShift(0) -idist + &
                              & P12_CoulShift(0))
                            DDUParam(271 + AIDI + 0) = DDUParam(271 + AIDI + 0) + Param(43) * Chargej * (-val5 + P12_UShift(1))
                            DDUParam(271 + AIDJ + 0) = DDUParam(271 + AIDJ + 0) + Param(43) * Chargei * (-val5 + P12_UShift(1))
                            DDUParam(269 + AIDI + 0) = DDUParam(269 + AIDI + 0) + Chargej * (val2 + P12_UShift(0) - idist + &
                              & P12_CoulShift(0))
                            DDUParam(269 + AIDJ + 0) = DDUParam(269 + AIDJ + 0) + Chargei * (val2 + P12_UShift(0) - idist + &
                              & P12_CoulShift(0))
                    ...
                    if (CALCDWPARAM) then
                        ...
                        if (AIDI==AIDJ) then
                            ...
                            DDWParam(271 + AIDI + 0) = DDWParam(271 + AIDI + 0) + 2.d0 * Chargej * Param(43) * val4
                            ...
                            DDWParam(271 + AIDI + 0) = DDWParam(271 + AIDI + 0) + Param(43) * val4 * Chargej
                            DDWParam(271 + AIDJ + 0) = DDWParam(271 + AIDJ + 0) + Param(43) * val4 * Chargei
            ...
            if (dijsq < CutSq(13)) then
                ...
                if (CALCDUPARAM) then
                    ...
                    if (AIDI==AIDJ) then
                        DUParam(AIDI) = DUParam(AIDI) + 2.d0 * Param(45) * Chargej * (val2 + P13_UShift(0) - idist + &
                          & P13_CoulShift(0))
                        DDUParam(275 + AIDI + 0) = DDUParam(275 + AIDI + 0) + 2.d0 * Param(45) * Chargej * (-val5 + &
                          & P13_UShift(1))
                        DDUParam(273 + AIDI + 0) = DDUParam(273 + AIDI + 0) + 2.d0 * Chargej * (val2 + P13_UShift(0) - idist + &
                          & P13_CoulShift(0))
                        ...
                        DDUParam(275 + AIDI + 0) = DDUParam(275 + AIDI + 0) + Param(45) * Chargej * (-val5 + P13_UShift(1))
                        DDUParam(275 + AIDJ + 0) = DDUParam(275 + AIDJ + 0) + Param(45) * Chargei * (-val5 + P13_UShift(1))
                        DDUParam(273 + AIDI + 0) = DDUParam(273 + AIDI + 0) + Chargej * (val2 + P13_UShift(0) - idist + &
                          & P13_CoulShift(0))
                        DDUParam(273 + AIDJ + 0) = DDUParam(273 + AIDJ + 0) + Chargei * (val2 + P13_UShift(0) - idist + &
                          & P13_CoulShift(0))
                ...
                if (CALCDWPARAM) then
                    ...
                    if (AIDI==AIDJ) then
                        ...
                        DDWParam(275 + AIDI + 0) = DDWParam(275 + AIDI + 0) + 2.d0 * Chargej * Param(45) * val4
                        DDWParam(273 + AIDI + 0) = DDWParam(273 + AIDI + 0) + 2.d0 * Chargej * (val6 - val2 + idist)
                        ...
                        DDWParam(275 + AIDI + 0) = DDWParam(275 + AIDI + 0) + Param(45) * val4 * Chargej
                        DDWParam(275 + AIDJ + 0) = DDWParam(275 + AIDJ + 0) + Param(45) * val4 * Chargei
                        DDWParam(273 + AIDI + 0) = DDWParam(273 + AIDI + 0) + Chargej * (val6 - val2 + idist)
                        DDWParam(273 + AIDJ + 0) = DDWParam(273 + AIDJ + 0) + Chargei * (val6 - val2 + idist)
...
subroutine calcargstats(Weight)
    ...
    !potential ExtSin
    P14_ArgWeightSumStats = P14_ArgWeightSumStats + Weight
    ...
    do i = istart, istop
        ...
        if (AIDi==1) then
            ...
            !factor = 2*PI*Param(48)/BOXL(2)
            if (Weight > 0.d0) then
                P14_ArgMin(ArgType) = min(P14_ArgMin(ArgType), ArgVal)
                P14_ArgMax(ArgType) = max(P14_ArgMax(ArgType), ArgVal)
                P14_ArgCount(ArgType) = P14_ArgCount(ArgType) + Weight
                P14_ArgSum(ArgType) = P14_ArgSum(ArgType) + ArgVal * Weight
                P14_ArgSumSq(ArgType) = P14_ArgSumSq(ArgType) + ArgVal * ArgVal * Weight
            endif
        end if
...
subroutine calcarghist(Weight)
    ...
    !potential ExtSin
    P14_ArgWeightSumHist = P14_ArgWeightSumHist + Weight
    ...
    do i = istart, istop
        ...
        if (AIDi==1) then
            ...
            !factor = 2*PI*Param(48)/BOXL(2)
            if (10000 > 0) then
                BinInd = int((ArgVal - P14_ArgHistMin(ArgType)) * P14_ArgHistiBinw(ArgType))
                if (BinInd >= 0 .and. BinInd < 10000) P14_ArgHist(ArgType,BinInd) = P14_ArgHist(ArgType,BinInd) + Weight
            endif
        end if
...
subroutine calcargeval(CalcForce, CalcVirial, CalcDUParam, CalcDWParam, CalcFluct)
        ...
        do i = 0, 10000 - 1
            ...
            if (CALCDUPARAM) then
                ...
                if (AIDI==AIDJ) then
                    DUParam(AIDI) = DUParam(AIDI) + (ThisHist) * (2.d0 * Param(35) * Chargej * (val2 + P8_UShift(0) - idist + &
                      & P8_CoulShift(0)))
                    DDUParam(255 + AIDI + 0) = DDUParam(255 + AIDI + 0) + (ThisHist) * (2.d0 * Param(35) * Chargej * (-val5 + &
                      & P8_UShift(1)))
                    DDUParam(253 + AIDI + 0) = DDUParam(253 + AIDI + 0) + (ThisHist) * (2.d0 * Chargej * (val2 + P8_UShift(0) - &
                    ...
                    DUParam(AIDJ) = DUParam(AIDJ) + (ThisHist) * (Param(35) * Chargei * (val2 + P8_UShift(0) -idist + &
                      & P8_CoulShift(0)))
                    DDUParam(255 + AIDI + 0) = DDUParam(255 + AIDI + 0) + (ThisHist) * (Param(35) * Chargej * (-val5 + &
                      & P8_UShift(1)))
                    DDUParam(255 + AIDJ + 0) = DDUParam(255 + AIDJ + 0) + (ThisHist) * (Param(35) * Chargei * (-val5 + &
                      & P8_UShift(1)))
                    DDUParam(253 + AIDI + 0) = DDUParam(253 + AIDI + 0) + (ThisHist) * (Chargej * (val2 + P8_UShift(0) - idist  &
                      & + P8_CoulShift(0)))
                    DDUParam(253 + AIDJ + 0) = DDUParam(253 + AIDJ + 0) + (ThisHist) * (Chargei * (val2 + P8_UShift(0) - idist  &
                      & + P8_CoulShift(0)))
            ...
            if (CALCDWPARAM) then
                ...
                if (AIDI==AIDJ) then
                    ...
                    DDWParam(255 + AIDI + 0) = DDWParam(255 + AIDI + 0) + (ThisHist) * (2.d0 * Chargej * Param(35) * val4)
                    ...
                    DDWParam(255 + AIDI + 0) = DDWParam(255 + AIDI + 0) + (ThisHist) * (Param(35) * val4 * Chargej)
                    DDWParam(255 + AIDJ + 0) = DDWParam(255 + AIDJ + 0) + (ThisHist) * (Param(35) * val4 * Chargei)
        ...
        do i = 0, 10000 - 1
            ...
            if (CALCDUPARAM) then
                ...
                if (AIDI==AIDJ) then
                    DUParam(AIDI) = DUParam(AIDI) + (ThisHist) * (2.d0 * Param(37) * Chargej * (val2 + P9_UShift(0) - idist + &
                      & P9_CoulShift(0)))
                    DDUParam(259 + AIDI + 0) = DDUParam(259 + AIDI + 0) + (ThisHist) * (2.d0 * Param(37) * Chargej * (-val5 + &
                      & P9_UShift(1)))
                    DDUParam(257 + AIDI + 0) = DDUParam(257 + AIDI + 0) + (ThisHist) * (2.d0 * Chargej * (val2 + P9_UShift(0) - &
                    ...
                    DUParam(AIDJ) = DUParam(AIDJ) + (ThisHist) * (Param(37) * Chargei * (val2 + P9_UShift(0) -idist + &
                      & P9_CoulShift(0)))
                    DDUParam(259 + AIDI + 0) = DDUParam(259 + AIDI + 0) + (ThisHist) * (Param(37) * Chargej * (-val5 + &
                      & P9_UShift(1)))
                    DDUParam(259 + AIDJ + 0) = DDUParam(259 + AIDJ + 0) + (ThisHist) * (Param(37) * Chargei * (-val5 + &
                      & P9_UShift(1)))
                    DDUParam(257 + AIDI + 0) = DDUParam(257 + AIDI + 0) + (ThisHist) * (Chargej * (val2 + P9_UShift(0) - idist  &
                      & + P9_CoulShift(0)))
                    DDUParam(257 + AIDJ + 0) = DDUParam(257 + AIDJ + 0) + (ThisHist) * (Chargei * (val2 + P9_UShift(0) - idist  &
                      & + P9_CoulShift(0)))
            ...
            if (CALCDWPARAM) then
                ...
                if (AIDI==AIDJ) then
                    ...
                    DDWParam(259 + AIDI + 0) = DDWParam(259 + AIDI + 0) + (ThisHist) * (2.d0 * Chargej * Param(37) * val4)
                    ...
                    DDWParam(259 + AIDI + 0) = DDWParam(259 + AIDI + 0) + (ThisHist) * (Param(37) * val4 * Chargej)
                    DDWParam(259 + AIDJ + 0) = DDWParam(259 + AIDJ + 0) + (ThisHist) * (Param(37) * val4 * Chargei)
        ...
        do i = 0, 10000 - 1
            ...
            if (CALCDUPARAM) then
                ...
                if (AIDI==AIDJ) then
                    DUParam(AIDI) = DUParam(AIDI) + (ThisHist) * (2.d0 * Param(39) * Chargej * (val2 + P10_UShift(0) - idist + &
                      & P10_CoulShift(0)))
                    DDUParam(263 + AIDI + 0) = DDUParam(263 + AIDI + 0) + (ThisHist) * (2.d0 * Param(39) * Chargej * (-val5 + &
                      & P10_UShift(1)))
                    DDUParam(261 + AIDI + 0) = DDUParam(261 + AIDI + 0) + (ThisHist) * (2.d0 * Chargej * (val2 + P10_UShift(0)  &
                      & - idist + P10_CoulShift(0)))
                    ...
                    DUParam(AIDJ) = DUParam(AIDJ) + (ThisHist) * (Param(39) * Chargei * (val2 + P10_UShift(0) -idist + &
                      & P10_CoulShift(0)))
                    DDUParam(263 + AIDI + 0) = DDUParam(263 + AIDI + 0) + (ThisHist) * (Param(39) * Chargej * (-val5 + &
                      & P10_UShift(1)))
                    DDUParam(263 + AIDJ + 0) = DDUParam(263 + AIDJ + 0) + (ThisHist) * (Param(39) * Chargei * (-val5 + &
                      & P10_UShift(1)))
                    DDUParam(261 + AIDI + 0) = DDUParam(261 + AIDI + 0) + (ThisHist) * (Chargej * (val2 + P10_UShift(0) - idist &
                      & + P10_CoulShift(0)))
                    DDUParam(261 + AIDJ + 0) = DDUParam(261 + AIDJ + 0) + (ThisHist) * (Chargei * (val2 + P10_UShift(0) - idist &
                      & + P10_CoulShift(0)))
            ...
            if (CALCDWPARAM) then
                ...
                if (AIDI==AIDJ) then
                    ...
                    DDWParam(263 + AIDI + 0) = DDWParam(263 + AIDI + 0) + (ThisHist) * (2.d0 * Chargej * Param(39) * val4)
                    ...
                    DDWParam(263 + AIDI + 0) = DDWParam(263 + AIDI + 0) + (ThisHist) * (Param(39) * val4 * Chargej)
                    DDWParam(263 + AIDJ + 0) = DDWParam(263 + AIDJ + 0) + (ThisHist) * (Param(39) * val4 * Chargei)
        ...
        do i = 0, 10000 - 1
            ...
            if (CALCDUPARAM) then
                ...
                if (AIDI==AIDJ) then
                    DUParam(AIDI) = DUParam(AIDI) + (ThisHist) * (2.d0 * Param(41) * Chargej * (val2 + P11_UShift(0) - idist + &
                      & P11_CoulShift(0)))
                    DDUParam(267 + AIDI + 0) = DDUParam(267 + AIDI + 0) + (ThisHist) * (2.d0 * Param(41) * Chargej * (-val5 + &
                      & P11_UShift(1)))
                    DDUParam(265 + AIDI + 0) = DDUParam(265 + AIDI + 0) + (ThisHist) * (2.d0 * Chargej * (val2 + P11_UShift(0)  &
                      & - idist + P11_CoulShift(0)))
                    ...
                    DUParam(AIDJ) = DUParam(AIDJ) + (ThisHist) * (Param(41) * Chargei * (val2 + P11_UShift(0) -idist + &
                      & P11_CoulShift(0)))
                    DDUParam(267 + AIDI + 0) = DDUParam(267 + AIDI + 0) + (ThisHist) * (Param(41) * Chargej * (-val5 + &
                      & P11_UShift(1)))
                    DDUParam(267 + AIDJ + 0) = DDUParam(267 + AIDJ + 0) + (ThisHist) * (Param(41) * Chargei * (-val5 + &
                      & P11_UShift(1)))
                    DDUParam(265 + AIDI + 0) = DDUParam(265 + AIDI + 0) + (ThisHist) * (Chargej * (val2 + P11_UShift(0) - idist &
                      & + P11_CoulShift(0)))
                    DDUParam(265 + AIDJ + 0) = DDUParam(265 + AIDJ + 0) + (ThisHist) * (Chargei * (val2 + P11_UShift(0) - idist &
                      & + P11_CoulShift(0)))
            ...
            if (CALCDWPARAM) then
                ...
                if (AIDI==AIDJ) then
                    ...
                    DDWParam(267 + AIDI + 0) = DDWParam(267 + AIDI + 0) + (ThisHist) * (2.d0 * Chargej * Param(41) * val4)
                    ...
                    DDWParam(267 + AIDI + 0) = DDWParam(267 + AIDI + 0) + (ThisHist) * (Param(41) * val4 * Chargej)
                    DDWParam(267 + AIDJ + 0) = DDWParam(267 + AIDJ + 0) + (ThisHist) * (Param(41) * val4 * Chargei)
        ...
        do i = 0, 10000 - 1
            ...
            if (CALCDUPARAM) then
                ...
                if (AIDI==AIDJ) then
                    DUParam(AIDI) = DUParam(AIDI) + (ThisHist) * (2.d0 * Param(43) * Chargej * (val2 + P12_UShift(0) - idist + &
                      & P12_CoulShift(0)))
                    DDUParam(271 + AIDI + 0) = DDUParam(271 + AIDI + 0) + (ThisHist) * (2.d0 * Param(43) * Chargej * (-val5 + &
                      & P12_UShift(1)))
                    DDUParam(269 + AIDI + 0) = DDUParam(269 + AIDI + 0) + (ThisHist) * (2.d0 * Chargej * (val2 + P12_UShift(0)  &
                      & - idist + P12_CoulShift(0)))
                    ...
                    DUParam(AIDJ) = DUParam(AIDJ) + (ThisHist) * (Param(43) * Chargei * (val2 + P12_UShift(0) -idist + &
                      & P12_CoulShift(0)))
                    DDUParam(271 + AIDI + 0) = DDUParam(271 + AIDI + 0) + (ThisHist) * (Param(43) * Chargej * (-val5 + &
                      & P12_UShift(1)))
                    DDUParam(271 + AIDJ + 0) = DDUParam(271 + AIDJ + 0) + (ThisHist) * (Param(43) * Chargei * (-val5 + &
                      & P12_UShift(1)))
                    DDUParam(269 + AIDI + 0) = DDUParam(269 + AIDI + 0) + (ThisHist) * (Chargej * (val2 + P12_UShift(0) - idist &
                      & + P12_CoulShift(0)))
                    DDUParam(269 + AIDJ + 0) = DDUParam(269 + AIDJ + 0) + (ThisHist) * (Chargei * (val2 + P12_UShift(0) - idist &
                      & + P12_CoulShift(0)))
            ...
            if (CALCDWPARAM) then
                ...
                if (AIDI==AIDJ) then
                    ...
                    DDWParam(271 + AIDI + 0) = DDWParam(271 + AIDI + 0) + (ThisHist) * (2.d0 * Chargej * Param(43) * val4)
                    ...
                    DDWParam(271 + AIDI + 0) = DDWParam(271 + AIDI + 0) + (ThisHist) * (Param(43) * val4 * Chargej)
                    DDWParam(271 + AIDJ + 0) = DDWParam(271 + AIDJ + 0) + (ThisHist) * (Param(43) * val4 * Chargei)
        ...
        do i = 0, 10000 - 1
            ...
            if (CALCDUPARAM) then
                ...
                if (AIDI==AIDJ) then
                    DUParam(AIDI) = DUParam(AIDI) + (ThisHist) * (2.d0 * Param(45) * Chargej * (val2 + P13_UShift(0) - idist + &
                      & P13_CoulShift(0)))
                    DDUParam(275 + AIDI + 0) = DDUParam(275 + AIDI + 0) + (ThisHist) * (2.d0 * Param(45) * Chargej * (-val5 + &
                      & P13_UShift(1)))
                    DDUParam(273 + AIDI + 0) = DDUParam(273 + AIDI + 0) + (ThisHist) * (2.d0 * Chargej * (val2 + P13_UShift(0)  &
                    ...
                    DUParam(AIDJ) = DUParam(AIDJ) + (ThisHist) * (Param(45) * Chargei * (val2 + P13_UShift(0) -idist + &
                      & P13_CoulShift(0)))
                    DDUParam(275 + AIDI + 0) = DDUParam(275 + AIDI + 0) + (ThisHist) * (Param(45) * Chargej * (-val5 + &
                      & P13_UShift(1)))
                    DDUParam(275 + AIDJ + 0) = DDUParam(275 + AIDJ + 0) + (ThisHist) * (Param(45) * Chargei * (-val5 + &
                      & P13_UShift(1)))
                    DDUParam(273 + AIDI + 0) = DDUParam(273 + AIDI + 0) + (ThisHist) * (Chargej * (val2 + P13_UShift(0) - idist &
                      & + P13_CoulShift(0)))
                    DDUParam(273 + AIDJ + 0) = DDUParam(273 + AIDJ + 0) + (ThisHist) * (Chargei * (val2 + P13_UShift(0) - idist &
                      & + P13_CoulShift(0)))
            ...
            if (CALCDWPARAM) then
                ...
                if (AIDI==AIDJ) then
                    ...
                    DDWParam(275 + AIDI + 0) = DDWParam(275 + AIDI + 0) + (ThisHist) * (2.d0 * Chargej * Param(45) * val4)
                    DDWParam(273 + AIDI + 0) = DDWParam(273 + AIDI + 0) + (ThisHist) * (2.d0 * Chargej * (val6 - val2 + idist))
                    ...
                    DDWParam(275 + AIDI + 0) = DDWParam(275 + AIDI + 0) + (ThisHist) * (Param(45) * val4 * Chargej)
                    DDWParam(275 + AIDJ + 0) = DDWParam(275 + AIDJ + 0) + (ThisHist) * (Param(45) * val4 * Chargei)
                    DDWParam(273 + AIDI + 0) = DDWParam(273 + AIDI + 0) + (ThisHist) * (Chargej * (val6 - val2 + idist))
                    DDWParam(273 + AIDJ + 0) = DDWParam(273 + AIDJ + 0) + (ThisHist) * (Chargei * (val6 - val2 + idist))
    ...
    do ArgType = 0, 1 - 1
        do i = 0, 10000 - 1
            ThisHist = P14_ArgHist(ArgType, i)
            if (ThisHist == 0.d0) cycle
            ArgVal = P14_ArgHistMin(ArgType) + P14_ArgHistBinw(ArgType) * (0.5d0 + i)
            ...
            !THISU = Param(47) * PlaneDist
            if (CalcDUParam) then
            !    DUParam(47) = DUParam(47) + (ThisHist) * (SIN(2*PI*(PlaneDist-P14_PlaneLoc)*Param(48)/BOXL(2)))
                DUParam(47) = DUParam(47) + (ThisHist) * (SIN(factor*(PlaneDist-P14_PlaneLoc)))
            endif
            PEnergy = PEnergy + ThisU * ThisHist
            Terms(14) = Terms(14) + ThisU * ThisHist
        enddo
    enddo
...
function modulehash()
    ...
    modulehash = 'df65e3e1a3b973a35ac54127eae064c701941ab5'
end function
